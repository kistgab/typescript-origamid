...
function saveObjectInLocalStorage(key: string, data: object) {
  localStorage.setItem(key, JSON.stringify(data));
}

function handleFormKeyUp({ target }: KeyboardEvent) {
  if (
...
function isUserData(data: unknown): data is IUserData {
  ...
  const hasUserDataProps =
    UserDataProps.nome in data ||
...
function fillFormData(formData: IUserData) {
...
function initializeEvent<K extends keyof HTMLElementEventMap>(
  id: string,
  eventName: K,
  callback: any
) {
  const formElement = document.querySelector<HTMLElement>(id);
